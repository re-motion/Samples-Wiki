...
// the original example that does not pass the 'IParrot' interface to Mixin<TThis>.
...
// with 'Parrot' for 'TThis'. The mixin class 'OnThePhoneMixin'
// uses the 'Say' method of the target class. 

namespace WikiSamples.ParrotMixinClass
{
  // target class
  public class Parrot
  {
  ...
  [Extends (typeof (Parrot))]  // specifies the target class for the mixin

  // 'Parrot' gives 'OnThePhoneMixin' a handle on 'Say'
  // via 'This'. 
  public class OnThePhoneMixin : Mixin<Parrot>, IOnThePhoneMixin
  {
